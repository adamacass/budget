          ...
          {pace && (
            <>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '0.75rem' }}>
                <span style={{ fontSize: '1.5rem', fontWeight: 700, color: pacePercent > 110 ? 'var(--red)' : pacePercent > 90 ? 'var(--yellow)' : 'var(--green)' }}>{fmtMoney(pace.daily_average)}/day</span>
                <div style={{ textAlign: 'right', fontSize: '0.8rem', color: 'var(--text-muted)' }}>Projected: {fmtMoney(pace.projected_monthly)}/mo<br />Budget: {fmtMoney(pace.monthly_budget)}/mo</div>
              </div>
              <div className="progress-bar" style={{ height: 12, marginBottom: '0.5rem' }}>
                <div className={`progress-fill ${pacePercent <= 90 ? 'green' : pacePercent <= 110 ? 'yellow' : 'red'}`} style={{ width: `${Math.min(100, pacePercent)}%` }} />
              </div>
              <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)', display: 'flex', justifyContent: 'space-between' }}>
                <span>{fmtMoney(pace.total_spent)} in {pace.days_elapsed}d</span>
                <span style={{ fontWeight: 600, color: pacePercent <= 100 ? 'var(--green)' : 'var(--red)' }}>{pacePercent <= 100 ? `${100 - pacePercent}% under` : `${pacePercent - 100}% over`}</span>
              </div>
            </>
          )}
        </div>
