...
| Migration script: `server version mismatch` from `pg_dump` | Your desktop's `pg_dump` is older than Render's Postgres | Uninstall/rename the local `pg_dump` so the script falls back to the containerised one, or upgrade your local Postgres client tools. |
